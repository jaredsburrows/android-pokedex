...
class PokemonServiceTest {
  ...
  @Test
  fun testFetchPokemonListURLShouldParseCorrectly() =
    runTest {
      val response = runBlocking(IO) { sut.fetchPokemonList(0, 20) }
  ...
  @Test
  fun testFetchPokemonInfoURLShouldParseCorrectly() =
    runTest {
      val pokemonName = "bulbasaur"
      val response = runBlocking(IO) { sut.fetchPokemonInfo(pokemonName) }
      val body = response.body()!!
